...
export default function AdminOperatorPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/operator/session', { credentials: 'include', cache: 'no-store' })
      .then(res => {
        if (!res.ok) { if (mounted) { setAuthChecking(false); router.replace('/business-login'); } return null; }
        return res.json();
      })
      .then(data => {
      ...
      .catch(() => { if (mounted) { setAuthChecking(false); router.replace('/business-login'); } });
    ...
    <main className="min-h-screen flex flex-col pb-24">
      ...
      <header className="flex items-center justify-between p-6 pt-8">
        <div className="fade-up">
          <p className="text-xs uppercase tracking-widest text-[var(--muted)]">{businessName || 'Fidelizat'} · Admin</p>
      ...
      <div className="px-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3 fade-up delay-200">
          <div className="glass-card rounded-2xl p-5 flex flex-col items-center gap-3 border border-[var(--border)] opacity-50 cursor-not-allowed select-none relative overflow-hidden">
