      ...
      <div className={`glass-card rounded-2xl p-6 w-full ${isComplete ? 'reward-glow' : ''}`}>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            <h3 className="font-display text-xl font-semibold text-[var(--text)]">
              {/^\d/.test(client.name) ? 'Client' : client.name}
            </h3>
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-[var(--muted)]">/ {visitGoal} vizite</p>
          </div>
        </div>

        {/* Mini stamp grid */}
        <div className="grid gap-1.5 mb-4" style={{ gridTemplateColumns: `repeat(${visitGoal}, minmax(0, 1fr))` }}>
          {Array.from({ length: visitGoal }, (_, stampNumber) => stampNumber + 1).map((stampNumber) => (
            <div
              key={stampNumber}
        ...
        {isComplete && (
          <div className="bg-[var(--gold-dim)]/10 border border-[var(--gold-dim)]/30 rounded-xl p-3 mb-4 text-center">
            <p className="text-sm font-semibold text-[var(--gold)]">🎉 Bonus deblocat! Gata de recompensat.</p>
          </div>
        )}
      ...
      {showRewardModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card reward-glow rounded-3xl p-8 max-w-sm w-full text-center fade-up">
            ...
            <h2 className="font-display text-2xl font-bold text-[var(--gold)] mb-2">Bonus deblocat!</h2>
            <p className="text-[var(--text-dim)] mb-6">{client.name} a atins {visitGoal} vizite și a câștigat o <strong className="text-[var(--text)]">recompensă gratuită</strong>!</p>
            <div className="flex flex-col gap-3">
              ...
              >
                {loading === 'claim' ? (
                  <div className="w-4 h-4 border-2 border-[var(--dark)] border-t-transparent rounded-full animate-spin mx-auto" />
                ) : 'Acordă bonus & Resetează'}
              </button>
              <button
